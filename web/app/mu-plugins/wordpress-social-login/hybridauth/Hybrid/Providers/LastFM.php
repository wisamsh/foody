...
class Hybrid_Providers_LastFM extends Hybrid_Provider_Model {
	...
	function initialize() {
		...
		$this->api = new LastFM( array(
			'api_key'    => $this->config["keys"]["key"],
			'api_secret' => $this->config["keys"]["secret"]
		) );
	...
	function getUserProfile() {
		try {
			$response = $this->api->api( "user.getInfo", array(
				"token" => $this->token( "access_token" ),
				"user"  => $this->token( "user_name" )
			) );
