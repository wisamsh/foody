...
function foody_get_coupon_value()
{
        ...
        if (strpos($coupon_code, '_') != false) {
            ...
            if (count($coupon_details) == 2) {
                ...
                if(isset($coupon->used) && $coupon->used == '0'){
                    $new_price = get_modified_course_price($course_id, $coupon->coupon_value);
                }
            }
        } else {
            /** general coupon */
    ...
    if(!empty($new_price)){
        ...
        wp_send_json_success(['wp_send_json_success' => 'no discount']);
    }
}
...
function  get_modified_course_price($course_id, $coupon_value){
