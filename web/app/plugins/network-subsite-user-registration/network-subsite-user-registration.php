            ...
            public function nsur_add_subsite_to_logged_in_user( ) {
                    ...
                    if ( $network_user_registration_configured     
                         && $local_join_site_enabled
							...
							$redirect_url = $query_args["redirect_to"];
...
/**
 * Store originating blog_id to a cookie for the confirmation page (wp-active.php) to follow correctly for the user.
 * if has been found necessary to break this code out of the NSUR class to fire at the right way.
 */
add_action( 'wpmu_activate_user' , 'my_hook_to_new_user', 10, 3 );

function my_hook_to_new_user (  $user_id, $password, $meta ) {

	// an extra to store originating locale to user meta for future use
