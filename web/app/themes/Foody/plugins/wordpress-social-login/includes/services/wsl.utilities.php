...
function wsl_display_dev_mode_debugging_area( $keyword = 'wsl_' ) {
    ...
    <table class="wsl-dev-table">
        <tr>
            <td colspan="3">
                1. SAVEQUERIES should be defined and set to TRUE in order for the queries to show up
                (http://codex.wordpress.org/Editing_wp-config.php#Save_queries_for_analysis)
                <br/>
                2. Calls for get_option() don't necessarily result on a query to the database. WP use both cache and
                wp_load_alloptions() to load all options at once. Hence, it won't be shown here.
            </td>
        </tr>
		<?php
                    ...
                    <tr>
                        <td valign="top" width="450">
							<?php if ( stristr( $caller_name, $keyword ) ): ?>
                                <a href="https://github.com/hybridauth/WordPress-Social-Login/search?q=<?php echo $caller_name; ?>"
                                   target="_blank" class="wsl-dev-wslfunc"><?php echo $caller_name; ?></a>
							<?php else: ?>
                                <a href="https://developer.wordpress.org/?s=<?php echo $caller_name; ?>" target="_blank"
                                   class="wsl-dev-nonwslfunc<?php if ( stristr( $caller_name, '_option' ) ) {
									   echo "- wsl-dev-optionfunc";
								   } ?>"><?php echo $caller_name; ?></a>
							<?php endif; ?>
                        ...
                        <td valign="top" class="<?php if ( ! stristr( '#' . $sql, '#select ' ) ) {
							echo 'wsl-dev-nonselectsql';
						} ?>"><?php echo nl2br( $sql ); ?></td>
                        <td valign="top" width="50" nowrap class="<?php if ( $time > 0.05 ) {
							echo 'wsl-dev-expensivesql';
						} ?>"><?php echo number_format( $time, 4, '.', '' ); ?></td>
                    </tr>
					<?php
				...
				if ( isset( $wp_filter[ $name ] ) ) {
                                        ...
                                        <tr>
                                            <td valign="top" width="270" nowrap class="wsl-dev-usedhook">
												...
												if ( stristr( $name, $keyword ) ) {
													?>
                                                    <a class="wsl-dev-usedwslhook"
                                                       href="https://github.com/hybridauth/WordPress-Social-Login/search?q=<?php echo $name; ?>"
                                            ...
                                            <td valign="top" class="wsl-dev-hookcallback">
												...
												if ( stristr( $callback['function'], $keyword ) ) {
													?>
                                                    <a href="https://github.com/hybridauth/WordPress-Social-Login/search?q=<?php echo $callback['function']; ?>"
                                                       target="_blank"><?php echo $callback['function']; ?></a>
													<?php
												} else {
                    ...
                    <tr>
                        <td valign="top" width="270" nowrap class="wsl-dev-unusedhook">
                            <a href="https://github.com/hybridauth/WordPress-Social-Login/search?q=<?php echo $name; ?>"
                               target="_blank"><?php echo $name; ?></a>
                        </td>
                        <td></td>
                        <td></td>
    ...
    <table class="wsl-dev-table">
		...
		foreach ( $_SESSION as $k => $v ) {
			?>
            <tr>
                <th width="270"><label><?php echo $k; ?></label></th>
                <td><?php print_r( $v ); ?></td>
            </tr>
			<?php
		}
    ...
    <table class="wsl-dev-table">
        <tbody>
        <tr>
            <th width="270"><label>Version</label></th>
            <td><?php echo get_bloginfo( 'version' ); ?></td>
        </tr>
        <tr>
            <th><label>Multi-site</label></th>
        ...
        <tr>
            <th><label>Plugins url</label></th>
            <td><?php echo plugins_url(); ?></td>
        </tr>
        </tbody>
    </table>
    ...
    <table class="wsl-dev-table">
        ...
        <tr>
            <th width="270"><label>Version</label></th>
            <td><?php echo wsl_get_version(); ?></td>
        </tr>
        <tr>
            <th><label>Plugin path</label></th>
            <td><?php echo WORDPRESS_SOCIAL_LOGIN_ABS_PATH; ?></td>
        </tr>
        <tr>
            <th><label>Plugin url</label></th>
            <td><?php echo WORDPRESS_SOCIAL_LOGIN_PLUGIN_URL; ?></td>
        </tr>
        <tr>
            <th><label>HA endpoint</label></th>
            <td><?php echo WORDPRESS_SOCIAL_LOGIN_HYBRIDAUTH_ENDPOINT_URL; ?></td>
        </tr>
        </tbody>
    </table>
    ...
    <table class="wsl-dev-table">
        <tbody>
        <tr>
            <th width="270"><label>IP</label></th>
        ...
        <tr>
            ...
            <td><?php echo isset( $_SERVER['SERVER_PORT'] ) ? 'On (' . $_SERVER['SERVER_PORT'] . ')' : 'N/A'; ?></td>
        </tr>
        <tr>
            <th><label>X Forward</label></th>
            <td><?php echo isset( $_SERVER['HTTP_X_FORWARDED_PROTO'] ) ? 'On (' . $_SERVER['HTTP_X_FORWARDED_PROTO'] . ')' : 'N/A';; ?></td>
        </tr>
        </tbody>
    </table>
    ...
    <table class="wsl-dev-table">
        <tbody>
        <tr>
            <th width="270"><label>Server</label></th>
        ...
        <tr>
            <th><label>MySQL</label></th>
            <td><?php echo $wpdb->db_version(); ?></td>
        </tr>
        <tr>
            <th><label>Time</label></th>
            <td><?php echo date( DATE_ATOM, time() ); ?> / <?php echo time(); ?></td>
        </tr>
        </tbody>
    </table>
    ...
    <table class="wsl-dev-table">
        <tbody>
        <tr>
            <th width="270"><label>Host</label></th>
        ...
        <tr>
            <th><label>Base_prefix</label></th>
            <td><?php echo $wpdb->prefix; ?></td>
        </tr>
        <tr>
            <th><label>Num_queries</label></th>
            <td><?php echo $wpdb->num_queries; ?></td>
        </tr>
        </tbody>
    </table>
	<?php
...
function wsl_generate_backtrace() {
	...
	for ( $i = 0; $i < $length; $i ++ ) {
		$result[] = ( $i + 1 ) . ')' . str_ireplace( array(
				realpath( WORDPRESS_SOCIAL_LOGIN_ABS_PATH ),
