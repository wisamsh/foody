			...
			foreach ( $WORDPRESS_SOCIAL_LOGIN_PROVIDERS_CONFIG AS $item ) {
				...
				if ( get_option( 'wsl_settings_' . $provider_id . '_enabled' ) ) {
					...
					if ( $provider_icon_markup != $provider_id ) {
                        ...
                        <a rel="nofollow" href="<?php echo $authenticate_url; ?>"
                           title="<?php echo sprintf( _wsl__( "Connect with %s", 'wordpress-social-login' ), $provider_name ) ?>"
                           ...
                           data-provider="<?php echo $provider_id ?>">
							<?php if ( $social_icon_set == 'none' ) {
								echo apply_filters( 'wsl_render_auth_widget_alter_provider_name', $provider_name );
							} else { ?><img alt="<?php echo $provider_name ?>"
                                            title="<?php echo sprintf( _wsl__( "Connect with %s", 'wordpress-social-login' ), $provider_name ) ?>"
                                            src="<?php echo $assets_base_url . strtolower( $provider_id ) . '.png' ?>" /><?php } ?>
	...
	if ( 'user_identities' == $meta ) {
		...
		if ( $linked_accounts ) {
			...
			foreach ( $linked_accounts AS $item ) {
				...
				?>
                <tr>
                <td><?php if ( $photourl ) { ?><img  style="vertical-align: top;width:16px;height:16px;"
                                                     src="<?php echo $photourl ?>"> <?php } else { ?><img
                        src="<?php echo $assets_base_url . strtolower( $item->provider ) . '.png' ?>" /> <?php } ?><?php echo ucfirst( $item->provider ); ?>
                </td>
                <td><?php echo $identity; ?></td></tr><?php
