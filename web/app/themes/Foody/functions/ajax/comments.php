...
add_action('wp_ajax_foody_increment_made_recipe', 'foody_increment_made_recipe');
add_action('wp_ajax_nopriv_foody_increment_made_recipe', 'foody_increment_made_recipe');
function foody_increment_made_recipe()
{
    $current_preps = get_post_meta($_POST['ID'], 'num_of_preps');
    $current_preps = is_array($current_preps) && !empty($current_preps) ? intval($current_preps[0]) : 0;

    update_post_meta($_POST['ID'], 'num_of_preps', $current_preps + 1);

    wp_send_json_success(['numOfPreps' => get_post_meta($_POST['ID'], 'num_of_preps')[0]]);
}
...
function foody_submit_ajax_how_i_did()
{
    ...
    $data = array(
        ...
        'comment_parent' => isset($_POST['comment_parent']) ? $_POST['comment_parent'] : '',
