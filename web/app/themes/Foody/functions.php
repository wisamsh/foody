...
function foody_scripts()
{
    ...
    if (!is_admin()) {
        ...
        if (
            is_page_template('page-templates/content-with-sidebar.php') && is_single()
...
function foody_body_add_bit_or_free_class($classes)
{
    ...
    if (get_post_type() == 'questions' || get_post_type() == 'poll') {
...
//HOOK FOR COMMENTS :=========================
add_filter('comment_text', 'ws_add_target_blank_to_auto_links', 20, 1);

function ws_add_target_blank_to_auto_links($comment_text) {
    // Use regex to find all <a> tags generated by WordPress
    $comment_text = preg_replace_callback(
        '/<a\s+([^>]+)>((?:.(?!<\/a>))+.)<\/a>/isU',
        function($matches) {
            // If the target="_blank" attribute is not already in the link, add it
            if (strpos($matches[1], 'target="_blank"') === false) {
                // Add target="_blank" to the link
                return '<a ' . $matches[1] . ' target="_blank">' . $matches[2] . '</a>';
            } else {
                // Return the link unchanged if target="_blank" already exists
                return $matches[0];
            }
        },
        $comment_text
