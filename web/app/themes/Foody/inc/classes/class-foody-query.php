...
class Foody_Query
{
    ...
    public function homepage()
    {
       ...
       //===============================================================
        ...
        if (get_current_blog_id() == 1 && !empty($featured) && (!empty($featured[0]) || !empty($featured[1]))) {
    ...
    public function search()
    {
        ...
        $args = self::get_args([
            'post_type' => ['foody_feed_channel', 'foody_recipe', 'foody_playlist', 'post'],
            's' => $search_term
        ]);
        ...
        if (!isset($_SESSION) || !isset($_SESSION['search_words_array']) || empty($_SESSION['search_words_array']) || (isset($_SESSION["LAST_ACTIVITY"]) && (time() - $_SESSION["LAST_ACTIVITY"] > 60))) {
            ...
            if (is_array($posts) && count($posts) > 0) {
                foreach ($posts as $post) {
                    $search_words_array[$post->post_title] = get_field('search_words', $post->ID);
                }
            }
