...
class WPGO_Foody_Sitemap_Shortcode {
	...
	public function render_sitemap( $attr ) {
		...
		$args = shortcode_atts( array(
			...
			'separator'            => 'false',
			'horizontal'           => 'false',
			// great for adding to footer
			'horizontal_separator' => ', ',
			'image'                => 'false',
			'shortcode_type'       => 'normal',
			'parent_page_link'     => '1',
			// is this used anymore, or only plugin settings version?
			'id'                   => '1'
			// if using multiple sitemaps with tabs then set this to a unique id for each sitemap to avoid CSS id conflicts
		), $attr );
