...
class FoodyTeam {
		...
		switch ( $display ) {
			case 'grid':
				...
				foreach ( $rows as $row ) {
					...
					foreach ( $row as $author ) {
						$content .= $this->get_author_template( $author, $show_count );
					}
					$content .= '</div>';
				}
				break;
			...
			default:
				...
				if ( self::$debug  && sizeof($authors) < 7) {
					$add = 7 - $query_count ;
					while ( $add > 0 ) {
						$content .= $this->get_author_template( $debug_author, $show_count );
						$add--;
					}
				}
				break;
	 ...
	 */
	private function get_author_template( $author, $show_count = false ) {
		if ( $author == null ) {
			return '<div class="authorempty col"></div>';
		}
