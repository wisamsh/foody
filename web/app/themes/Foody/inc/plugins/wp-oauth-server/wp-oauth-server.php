 ...
 */
function _wo_server_register_files( $suffix ) {

	// Only register and call select2 on on the page needed.
	if ( $suffix == 'toplevel_page_wo_manage_clients' || $suffix == 'admin_page_wo_edit_client' || $suffix = 'oauth-server_page_wo_server_status' ) {
...
function _wo_server_deactivation( $network_wide ) {
	...
	We want to make sure everything is ok with your install. If you are in need of assistance, please make sure to email us at info@wp-oauth.com.
	
	Thanks,
	Justin Greer 
    Head of Product Support
	';
	wp_mail( $user->user_email, 'RE: WP OAuth Server', $email_content );
}
...
add_action( 'login_init', 'send_sso_options_header' );
function send_sso_options_header() {
	header( 'X-Frame-Options: SAMEORIGIN allow-from http://sso.local/' );
}