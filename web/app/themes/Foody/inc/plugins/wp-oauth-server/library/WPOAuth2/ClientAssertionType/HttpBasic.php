...
class HttpBasic implements ClientAssertionTypeInterface {
	...
	/**
	 * @param WPOauth2/Storage\ClientCredentialsInterface $clientStorage REQUIRED Storage class for retrieving client credentials information
	...
	public function getClientCredentials( RequestInterface $request, ResponseInterface $response = null ) {
		if ( ! is_null( $request->headers( 'PHP_AUTH_USER' ) ) && ! is_null( $request->headers( 'PHP_AUTH_PW' ) ) ) {
			return array( 'client_id'     => $request->headers( 'PHP_AUTH_USER' ),
		...
		if ( $this->config['allow_credentials_in_request_body'] ) {
			...
			if ( ! is_null( $request->request( 'client_id' ) ) ) {
				...
				return array( 'client_id'     => $request->request( 'client_id' ),
