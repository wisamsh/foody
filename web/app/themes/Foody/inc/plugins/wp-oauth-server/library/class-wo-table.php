...
class WO_Table extends WP_List_Table {
	...
	public function prepare_items() {
		...
		/*
		 * Remove User Generated results form the client list.
		 *
		 * @Since 3.8.0
		 *
		 * @todo Add this feature to an option. Use case may need to see these in the aditor.
		 */
		$filtered   = $wpdb->get_results( $query );
		foreach ( $filtered as $key => $value ) {
			if ( strpos( $value->post_name, 'user_generated_' ) !== false ) {
				unset( $filtered[ $key ] );
			}
		}
