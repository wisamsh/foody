...
$config     = array(
	...
	// Use the same setting as use_jwt_access_tokens
...
if ( 'openid-configuration' == $well_known || 'oauth-authorization-server' == $well_known ) {
	$openid_discovery_values = apply_filters( 'wpo_well_known_openid_configuration', array(
			...
			'revocation_endpoint'                   => home_url( '/oauth/destroy/' ),
...
//if ( ! $server->verifyResourceRequest( WPOAuth2\Request::createFromGlobals() ) ) {
//	exit( "Failed" );
//}
//exit( "Success!" );
