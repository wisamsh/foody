...
function wp_oauth_profile_oauth_info( $user ) {
	...
	?>
    <h3>Open Authentication / Application Password</h3>
    <div>
		...
		<?php else: ?>
            ...
            <table class="form-table">
                <tr>
                    <th>Access Token</th>
                    <td><?php print $user_token->access_token; ?> <br/> <br/>
                        <input type="submit" name="generate_token" id="submit" class="button button-secondary"
                               value="Regenerate Token">
                        | <a href="javscript:void();" id="revoke-token">Revoke Token</a>
                    </td>
                </tr>
            </table>
...
function wo_user_profile_update_token_gen( $errors, $update, $user ) {
	if ( ! empty( $_POST['generate_token'] ) && is_user_logged_in() ) {
...
function wo_ap_token_gen_set_access_token( $access_token, $client_id, $user_id, $expires, $scope = null ) {
	 ...
	 */
	$insert = $wpdb->insert( "{$wpdb->prefix}oauth_access_tokens", array(
