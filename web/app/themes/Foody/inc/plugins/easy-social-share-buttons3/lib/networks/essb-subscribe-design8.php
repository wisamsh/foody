...
if (!function_exists('essb_subscribe_form_design8')) {
	function essb_subscribe_form_design8($salt, $is_widget = false, $position = '') {
		global $essb_options;
		
		$subscribe_mc_namefield = ESSBOptionValuesHelper::options_bool_value($essb_options, 'subscribe_mc_namefield8');
		...
		$subscribe_mc_title = ESSBOptionValuesHelper::options_value($essb_options, 'subscribe_mc_title8');
		$subscribe_mc_text = ESSBOptionValuesHelper::options_value($essb_options, 'subscribe_mc_text8');
		$subscribe_mc_email = ESSBOptionValuesHelper::options_value($essb_options, 'subscribe_mc_email8');
		$subscribe_mc_name = ESSBOptionValuesHelper::options_value($essb_options, 'subscribe_mc_name8');
		$subscribe_mc_button = ESSBOptionValuesHelper::options_value($essb_options, 'subscribe_mc_button8');
		$subscribe_mc_footer = ESSBOptionValuesHelper::options_value($essb_options, 'subscribe_mc_footer8');
		$subscribe_mc_success = ESSBOptionValuesHelper::options_value($essb_options, 'subscribe_mc_success8');
		$subscribe_mc_error = ESSBOptionValuesHelper::options_value($essb_options, 'subscribe_mc_error8');
		...
		$subscribe_mc_image3 = ESSBOptionValuesHelper::options_value($essb_options, 'subscribe_mc_image8');
		$subscribe_mc_imagealign3 = ESSBOptionValuesHelper::options_value($essb_options, 'subscribe_mc_imagealign8');
		
		if ($subscribe_mc_imagealign3 == '') {
			$subscribe_mc_imagealign3 = 'left';
		}
		
		if ($subscribe_mc_image3 == '') {
			$subscribe_mc_image3 = ESSB3_PLUGIN_URL . '/assets/images/templates/subscribe8.svg';
			$subscribe_mc_imagealign3 = 'left';
		}
		...
		$output = '<div class="essb-subscribe-form-content essb-subscribe-from-design8'.($is_widget ? " essb-subscribe-form-inwidget" :"").'" data-position="'.$position.'">';
