...
class ESSBShortcodeGenerator5 {
	...
	public function get_stored_key($key) {
		$saved_codes = get_option($this->shortcode_cache_option);
		
		if (!is_array($saved_codes)) {
			$saved_codes = array();
		}
		
		$has_data = isset($saved_codes[$key]) ? $saved_codes[$key] : array();
		
		$this->stored_shortcode_values = isset($has_data['settings']) ? $has_data['settings'] : array();
	}
	
	public function remove_stored_key($key) {
		$saved_codes = get_option($this->shortcode_cache_option);
	...
	function render() {
		...
		if ($ukey != '') {
			$this->get_stored_key($ukey);
			$this->stored_values_loaded = true;
			
			//print_r($this->stored_shortcode_values);
		}
		else {
			$this->stored_shortcode_values = array();
			$this->stored_values_loaded = false;
		}
	...
	function renderCheckbox($param, $settings, $cnt) {
		...
		$checkstate = "";
		if ($this->stored_values_loaded) {
			$hasvalue = isset($this->stored_shortcode_values[$param]) ? $this->stored_shortcode_values[$param] : '';
			if ($hasvalue != '') {
		...
		echo '<td class="bold" valign="top">'.$text.'<br/><span class="label">'.$comment.'</span></td>';
		echo '<td class="essb_general_options">';
		echo '<input id="'.$param.'" type="checkbox" name="'.$this->optionsGroup.'['.$param.']" '.$checkstate.' value="' . $default_value . '" />';
		echo '</td>';
		echo '</tr>';
	}
	...
	function renderDropDown($param, $settings, $cnt) {
		...
		$value = '';
		if ($this->stored_values_loaded) {
			$value = isset($this->stored_shortcode_values[$param]) ? $this->stored_shortcode_values[$param] : '';
		}
	...
	$text = isset($settings['text']) ? $settings['text'] : '';
		...
		if (is_array ( $essb_networks )) {
			foreach ( $essb_networks as $k => $v ) {
				...
				if ($this->stored_values_loaded) {
					$saved_value = isset($this->stored_shortcode_values[$k.'_name']) ? $this->stored_shortcode_values[$k.'_name'] : '';
				}
					
				echo '<li><p style="margin: .2em 5% .2em 0;">
						<input id="network__name_' . $k . '" name="'.$this->optionsGroup.'[' . $k . '_text]" type="text"
								class="input-element" value="'.$saved_value.'" />
	...
	function renderNetworkSelection($param, $settings, $cnt) {
		...
		if ($this->stored_values_loaded) {
			$selected_networks = isset($this->stored_shortcode_values[$param]) ? $this->stored_shortcode_values[$param] : '';
		}
		...
		if (is_array ( $essb_networks )) {
			foreach ( $essb_networks as $k => $v ) {
				...
				$is_checked = in_array($k, $selected_networks) ? ' checked="checked"' : '';
				$network_name = isset ( $v ['name'] ) ? $v ['name'] : $k;
...
jQuery(document).ready(function(){
	...
	public function store_shortcode($options, $shortcode_name) {
		$saved_codes = get_option($this->shortcode_cache_option);
		if (!is_array($saved_codes)) {
			$saved_codes = array();
		}
		...
		$has_key = isset($_REQUEST['ukey']) ? $_REQUEST['ukey'] : '';
		if ($has_key != '') {
			$key = $has_key;
		}
	...
	public function generate_as_options($options) {
		$r = array();
		...
		foreach ($this->shortcodeOptions as $param => $settings) {
			...
			if ($param == "counters" && $value == '') {
				$value = '0';
			}
			if ($param == "url" && trim($value) != '') {
				$exist_url = true;
			}
				
			if ($value != '') {
		
				if ($param != "network_names") {
					$r[$param] = $value;
				}
			}
		}
	...
	public function generate($options) {
		...
		$key_name = isset($options['shortcode_name']) ? $options['shortcode_name'] : '';
		if ($key_name != '') {
			$unique_key = $this->store_shortcode($options, $key_name);
		}
		...
		foreach ($this->shortcodeOptions as $param => $settings) {
			...
			if ($param == 'shortcode_name') { continue; }
		...
		echo '<div class="essb-shortcode-title essb-shortcode-titlegen">Your generated shortcode is</div>';
		echo '<div class="essb-shortcode">';
		echo $output;
		echo '</div>';
		
		echo '<div class="essb-shortcode-title essb-shortcode-titlegen">Include your shortcode into template files using this sample code</div>';
		...
		if ($unique_key != '') {
			echo '<div class="essb-shortcode-title essb-shortcode-titlegen">Use your shortcode with the stored unique key inside content</div>';
			echo '<div class="essb-shortcode">';
			echo '['.$this->shortcode.' ukey="'.$unique_key.'"]';
			echo '</div>';

			echo '<div class="essb-shortcode-title essb-shortcode-titlegen">Use your shortcode with the stored unique key inside content</div>';
			echo '<div class="essb-shortcode-code"><code>';
			echo '&lt;?php&nbsp;';
			echo 'echo do_shortcode(\'['.$this->shortcode.' ukey="'.$unique_key.'"]\');&nbsp;';
			echo '?&gt;&nbsp;</code>';
			echo '</div>';
		}
	}
	...
	public function activate($shortcode = 'easy-social-share') {
		...
		if ($shortcode == 'easy-fans' || $shortcode == 'easy-followers') {
			$this->includeOptionsForEasyFans('easy-followers', 'Shortcode to display followers counter');
		}
	...
	private function includePopularPosts() {
		...
		$this->shortcodeTitle = '[easy-popular-posts] Shortcode to display list of popular posts';
	...
	private function includeOptionsForTotalFollowers() {
		...
		$this->shortcodeTitle = '[easy-total-followers] Shortcode to display total number of followers (no options)';
	...
	private function includeOptionsForEasyProfiles() {
		...
		$this->shortcodeTitle = '[easy-profiles] Shortcode to display social profiles';
	...
	private function includeOptionsForTotalShares() {
		...
		$this->shortcodeTitle = '[easy-total-shares] Shortcode to display total number of shares';
	...
	private function includeOptionsForEasyShare() {
		...
		$this->shortcodeTitle = '[easy-social-share] Shortcode to display share buttons';
	...
	private function includeOptionsForEasyLike() {
		...
		$this->shortcodeTitle = '[easy-social-like] Shortcode to display native social buttons';
	...
	private function includeOptionsForEasySharePopup() {
		...
		$this->shortcodeTitle = '[easy-social-share-popup] Shortcode to display share buttons as pop up';
	...
	private function includeOptionsForEasyShareFlyin() {
		...
		$this->shortcodeTitle = '[easy-social-share-flyin] Shortcode to display share buttons as fly in';
