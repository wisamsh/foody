...
function wsl_component_contacts_list( $user_id ) {
    ...
    <table cellspacing="0" class="wp-list-table widefat fixed users">
		...
		if ( ! $user_contacts ) {
			?>
            <tr class="no-items">
                <td colspan="4" class="colspanchange"><?php _wsl_e( "No contacts found", 'wordpress-social-login' ) ?>
                    .
                </td>
            </tr>
			<?php
                ...
                <tr class="<?php if ( ++ $i % 2 )
                    ...
                    <td>
						<?php if ( $item->photo_url ) { ?>
                            <img width="32" height="32" class="avatar avatar-32 photo" align="middle"
                                 src="<?php echo $item->photo_url ?>">
						<?php } else { ?>
                            <img width="32" height="32" class="avatar avatar-32 photo" align="middle"
                                 src="http://www.gravatar.com/avatar/<?php echo md5( strtolower( trim( $item->email ) ) ); ?>">
						<?php } ?>
                    ...
                    <td>
						<?php if ( $item->profile_url ) { ?>
                            <a href="<?php echo $item->profile_url ?>" target="_blank"><?php echo str_ireplace( array(
									"http://www.",
									"https://www.",
									"http://",
									"https://"
								), array( '', '', '', '', '' ), $item->profile_url ) ?></a>
						<?php } else { ?>
                            -
						<?php } ?>
                    </td>
