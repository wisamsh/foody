...
function wsl_component_users_list() {
    ...
    <table cellspacing="0" class="wp-list-table widefat fixed users">
		...
		if ( ! $users_list ) {
			?>
            <tr class="no-items">
                <td colspan="5" class="colspanchange"><?php _wsl_e( "No users found", 'wordpress-social-login' ) ?>.
                </td>
            </tr>
			<?php
		} else {
			foreach ( $users_list as $items ) {
				...
				?>
                <tr class="<?php if ( ++ $i % 2 )
					echo "alternate" ?> tr-contacts">
                    <td nowrap>
						<?php
						foreach ( $linked_accounts AS $link ) {
							?>
                            <img src="<?php echo $assets_base_url . strtolower( $link->provider ) . '.png' ?>"
                                 style="vertical-align:top;width:16px;height:16px;"/> <?php _wsl_e( $link->provider, 'wordpress-social-login' ) ?>
                            <br/>
							<?php
                    ...
                    <td class="column-author">
						<?php if ( $wsl_user_image ) { ?>
                            <img width="32" height="32" class="avatar avatar-32 photo"
                                 src="<?php echo $wsl_user_image ?>">
						<?php } else { ?>
                            <img width="32" height="32" class="avatar avatar-32 photo"
                                 src="http://www.gravatar.com/avatar/<?php echo md5( strtolower( trim( $user_data->user_email ) ) ); ?>">
						<?php } ?>
                        ...
                        <div class="row-actions">
                            ...
                            <span class="delete">
								...
								?>
								<a style="color: #a00;" href="<?php echo $delete_url ?>"
                                   onClick="return confirmDeleteWSLUser();"><?php _wsl_e( "Delete", 'wordpress-social-login' ) ?></a>
							</span>
                        </div>
                    </td>
                    ...
                    <td>
						<?php if ( $user_data->user_url ) { ?>
                            <a href="<?php echo $user_data->user_url; ?>"
                               target="_blank"><?php echo str_ireplace( array(
									"http://www.",
									"https://www.",
									"http://",
									"https://"
								), array( '', '', '', '', '' ), $user_data->user_url ) ?></a>
						<?php } else { ?>
                            -
						<?php } ?>
                    </td>
                    <td align="center">
						...
						?>
                    <td align="center"><a class="thickbox"
                                          href="<?php echo admin_url( 'user-edit.php?user_id=' . $user_data->ID . '&TB_iframe=true&width=1150&height=550' ); ?>"><?php echo $user_data->ID; ?></a>
                    </td>
                </tr>
				<?php
			}
