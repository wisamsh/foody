...
window.calculator = function (selector) {
    ...
    let $numberOfDishes = $('.recipe-ingredients-top-old .amount-container .amount');

    let subIng = $('.recipe-ingredients-container-old .substitute-ingredient');
    ...
    let bundleIngredients = $('.recipe-ingredients-container-old .ingredient-container').find('[data-substitute-bundle=1]');
    ...
    subIng.on('click', function (event) {
            let parentIngredient = $(this).closest('.recipe-ingredients-container-old .ingredients');
            ...
            $elements = $('.recipe-ingredients-container-old li:not(.free-text-ingredients) .amount');
        }
    );
...
function handleSubsTextBackAndForth(textToShow, textForOriginal, caloriesNutrientOriginalVal, sugarNutrientOriginalVal, textColor, switchToSub) {
    ...
    if (showText) {
        ...
        if (!$('.difference-nutrient').length) {
            $('.recipe-ingredients-top-old').after('<span class="difference-nutrient" style="color:' + textColor + '">' + text + '</span>');
...
function handleSubsText(textToShow, caloriesNutrientOriginalVal, sugarNutrientOriginalVal, textColor) {
    ...
    if (showText) {
        if (!$('.difference-nutrient').length) {
            $('.recipe-ingredients-top-old').after('<span class="difference-nutrient" style="color:' + textColor + '">' + textToShow + '</span>');
